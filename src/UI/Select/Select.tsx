import React, { FC, useRef, useEffect, ReactHTMLElement, useState } from 'react';
import './Select.scss';
...
interface ISelect {
    ...
    setActiveNewElement?: () => void;
}

const Select: FC<ISelect> = ({ placeholder, height = 44, items, currentElement, setCurrentElement, addNewElement = false, setActiveNewElement }) => {
    const [active, setActive] = useState<boolean>(false);
    const [inputValue, setInputValue] = useState<string>('');
    const [elements, setElements] = useState<item[]>(items);
    const [tempCurrentElement, setTempCurrentElemtn] = useState<item | null>(null);
    ...
    useEffect(() => {
        setElements(items);
        if (inputValue) {
            setElements(elements.filter(el => el.name.toLowerCase().includes(inputValue.toLowerCase())));
        }
        if (currentElement) {
    ...
    return (
        ...
        >
            <div className='select__body'>
                ...
                {addNewElement && (
                    <button onClick={() => {setActiveNewElement && setActiveNewElement()}} className='select__add'>
            ...
            >
                {elements.length > 0 ? (
                    elements.map(item => (
                        <li key={item.name} onClick={() => onClickElement(item)} className='select__item'>
                            {item.name}
                        </li>
                    ))
                ) : (
