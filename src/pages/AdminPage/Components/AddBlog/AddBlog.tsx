...
import { BlogContentType, IBlog } from "../../../../models/Blog";
      ...
      >
            ...
            paragraphs.map((item, i) => (
              <li key={i} className='add-blog__paragraph'>
                <div className='add-blog__item'>
                  <p className='add-blog__text'>{item.text}</p>
                  {item.image && (
                    <span className='add-blog__img-name'>
                      {item.image.name.length < 30
                        ? "Изображение - " + item.image?.name
                        : "Изображение - " + item.image?.name.substring(0, 30)}
                    </span>
                  )}
                </div>
                <button onClick={() => {}} className='add-blog__cancel'>
                  <GiCancel size={20} />
                </button>
              </li>
            ))
        ...
        {img ? (
          <div className='add-blog__img'>
            <span className='add-blog__img-name'>
              {img.name.length < 30 ? img.name : img.name.substring(0, 30)}
            </span>
            <button onClick={() => setImg(null)} className='add-blog__cancel'>
              <GiCancel size={20} />
            </button>
          </div>
        ) : (
